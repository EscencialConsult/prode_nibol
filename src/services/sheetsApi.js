...
// ── Autenticación por CI (colaboradores) o email (admins) ──
// Los colaboradores NO usan email real: se autentican con su CI, que
// internamente se convierte al email ficticio {ci normalizado}@nibol.local.
// Los admins entran con su email real (prodenibol@gmail.com), que se usa
// tal cual. La decisión depende de si lo ingresado contiene "@".
// Normalización del CI: trim + minúsculas + sin espacios.
//   "9647252-1B"            → "9647252-1b@nibol.local"
//   "7699198"               → "7699198@nibol.local"
//   "prodenibol@gmail.com"  → "prodenibol@gmail.com" (email real, tal cual)
...
// ── auth ──────────────────────────────────────────────────
const auth = {
  // Recibe el CI (no el email). Internamente arma el email ficticio.
  login: async (ci, password) => {
    const email = ciToEmail(ci)
  ...
  // Registro espontáneo: recibe CI (no email). Arma el email ficticio
  // y guarda nombre + ci en el metadata. Como NO lleva el flag `migrado`,
  // el trigger handle_new_user lo deja en estado 'pendiente'.
  registro: async (nombre, ci, password) => {
    const email = ciToEmail(ci)
    const { data, error } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { nombre: nombre.trim(), ci: String(ci || '').trim() } },
    })
    if (error) throw new Error(traducirErrorAuth(error))
    return {
      ok: true,
...
function traducirErrorAuth(error) {
  ...
  if (msg.includes('invalid login credentials')) return 'CI o contraseña incorrectos'
  if (msg.includes('email not confirmed'))       return 'Tu cuenta no fue confirmada todavía'
  if (msg.includes('user already registered'))   return 'El CI ya está registrado'
...
const config = {
  // Lee una clave de la tabla `config`. Devuelve el valor (string) o null.
  obtener: async (clave) => {
    const { data, error } = await supabase
      .from('config').select('valor').eq('clave', clave).maybeSingle()
    if (error) {
      checkError(error, 'config.obtener')
      return null
    }
    return data?.valor ?? null
  },

  // Devuelve el plan de la empresa (ej: 'plan_basic'). null si no está seteado.
  plan: async () => {
    try {
      return await config.obtener('plan_empresa')
    } catch (e) {
      return null
    }
  },
...
const sheetsApi = {
  ...
  areas,
  config,
  ciToEmail, // helper CI → email ficticio
