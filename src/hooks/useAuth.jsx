...
export function AuthProvider({ children }) {
  ...
  // Plan de la empresa leído de config.plan_empresa (fuente global).
  // null = todavía no resuelto → usamos fallback de user.empresa.
...
const isAdmin = !!(
  ...
  // Leer el plan global desde config.plan_empresa cuando hay sesión.
  // Es la fuente de verdad; user.empresa queda como fallback.
  useEffect(() => {
    let cancelado = false
    if (!user) { setPlanEmpresa(null); return }
    sheetsApi.config.plan()
      .then(valor => { if (!cancelado && valor) setPlanEmpresa(valor) })
      .catch(() => {})
    return () => { cancelado = true }
  }, [user])

  // Plan de la empresa.
  // Prioridad: config.plan_empresa (global) → user.empresa (fallback).
  // Si nada está seteado, se considera Plan_pro por defecto
  // (mismo criterio que el backend en esPlanBasic_).
  const plan = String(planEmpresa || user?.empresa || '').trim().toLowerCase()
  const isPlanBasic = plan === 'plan_basic'
